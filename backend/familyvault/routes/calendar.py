...
def list_events(
    ...
    cal = get_or_404(db, Calendar, cal_id)
    require_role(db, cal.family_id, user.id, 'guest')
    query = select(Event).where(Event.calendar_id == cal_id)
    if from_:
        query = query.where(Event.start_at >= from_)
    if to:
        query = query.where(Event.end_at <= to)
...
def create_event(cal_id: int, payload: EventIn, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    cal = get_or_404(db, Calendar, cal_id)
    require_role(db, cal.family_id, user.id, 'adult')
    event = Event(calendar_id=cal_id, created_by=user.id, **payload.model_dump())
    db.add(event)
    db.commit()
    db.refresh(event)
    return event
...
def update_event(event_id: int, payload: EventIn, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    cal = get_or_404(db, Calendar, event.calendar_id)
    require_role(db, cal.family_id, user.id, 'adult')
